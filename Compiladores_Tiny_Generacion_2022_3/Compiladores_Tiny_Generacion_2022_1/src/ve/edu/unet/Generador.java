...
public class Generador {
	...
	private static void generarFor(NodoBase nodo){
		...
		localidadSaltoInicio = UtGen.emitirSalto(0);
		generar(n.getPrueba());
		localidadCondicionFor = UtGen.emitirSalto(1);
		generar(n.getCuerpo());
		...
		UtGen.emitirRM_Abs("JEQ", UtGen.AC, localidadFinal , "For: jmp hacia el final del cuerpo del for");
		UtGen.restaurarRespaldo();
		if(UtGen.debug)	UtGen.emitirComentario("<- for");
	}
	...
	private static void generarAsignacion(NodoBase nodo) {
		...
		if (n.getVariable() instanceof NodoVector) {
			...
			UtGen.emitirRM("LDA", UtGen.AC, direccion, UtGen.GP,
					"cargar direccion de identificador: " + identificador_vector.getNombre());
			...
			UtGen.emitirRM_Abs("IXA", UtGen.AC, direccion,
					"cargar direccion de identificador: " + identificador_vector.getNombre());
			generar(n.getExpresion());
			UtGen.emitirRM_Abs("STO", UtGen.AC, direccion,
					"asignacion: almaceno el valor para el id " + identificador_vector.getNombre());
		}
	}
	...
	private static void generarLeer(NodoBase nodo) {
		...
		if (n.getVariable() instanceof NodoIdentificador) {
			...
			UtGen.emitirRM_Abs("IXA", UtGen.AC, direccion,
					"cargar la direccion de la posicion del vector: " + id.getNombre());
	...
	private static void generarOperacion(NodoBase nodo) {
		...
		switch (n.getOperacion()) {
			...
			case and:
				...
				UtGen.emitirRM("JNE", UtGen.AC, 2, UtGen.PC, "voy dos instrucciones más allá no es igual a 1 (falso)");
				UtGen.emitirRM("LDC", UtGen.AC, 1, UtGen.AC, "caso de verdadero (AC = 1)");
				UtGen.emitirRM("LDC", UtGen.AC, 0, UtGen.AC, "caso de verdadero (AC = 0)");
				break;
			case or:
				UtGen.emitirRO("ADD", UtGen.AC, UtGen.AC1, UtGen.AC, "op: OR");
				UtGen.emitirRM("JNE", UtGen.AC, 2, UtGen.PC, "voy dos instrucciones más allá si no es igual a 1 (verdadero)");
				UtGen.emitirRM("LDC", UtGen.AC, 0, UtGen.AC, "caso de falso (AC = 0)");
				UtGen.emitirRM("LDC", UtGen.AC, 1, UtGen.AC, "caso de verdadero (AC = 1)");
				break;
			default:
